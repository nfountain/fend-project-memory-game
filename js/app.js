...
let flippedCards = [];
/* Idea to create an empty array to hold the flipped cards is courtesy of Matt
Cranford's blog, accessed 06/19/18 at
...
/* Resources: Card shuffle function came pre-loaded by Udacity; originally from
...
function resetCards() {
    ...
    for(let i = 0; i < listOfCards.length; i++) {
        ...
        /* Using an empty array to call the forEach method is courtesy of
        sandraisreal accessed 06/24/18, at 
...
/* Resources: firstElementChild.classList did not work, so used Matt Cranford's
approach of firstElementChild.className to find matches. Accessed his write-up
of the code on 06/24/18 at
<https://matthewcranford.com/memory-game-walkthrough-part-3-matching-pairs/> */
...
/* hideStar() for...of loop courtesy of Matt Cranford's blog accessed 07/03/18,
at <https://matthewcranford.com/memory-game-walkthrough-part-5-moves-stars/> */
function hideStar() {
    for(star of stars) {
...
// Clock/Timer
/* Resources: Memory Game Webinar with Ryan Waite used to help write the timer.
Accessed 06/30/18, at
<https://www.youtube.com/watch?v=oECVwum-7Zc&list=PL-UQ6doHcWnLwHtzKo_kGkZo9AcKI9KhL&index=12&t=2854s>. */
function startTime() {
...
// WIN GAME FUNCTIONS
// Check to make sure the whole deck has been matched
function allMatched() {
...
// Modal
/* Resource: Used code from a Traversy Media video titled "Create a Modal with
HTML, CSS, and JavaScript" viewed on 07/07/18, at
...
// EVENT LISTENERS
/* Resources: syntax for event listeners is from W3Schools on 06/24/18
...
/* the validClick function (to prevent counting clicks on the same card more
than once and prevent counting clicks on cards that are already matched) is
from Matt Cranford's blog, accessed 07/08/18, at
<https://matthewcranford.com/memory-game-walkthrough-part-3-matching-pairs/>. */
...
deckOfCards.addEventListener('click', function(event) {
    ...
    if(evtTarget.nodeName === 'LI') {
        if(validClick(evtTarget)) {
            if(timerOff) {
                /* function to turn the timer on with the first click courtesy
                of Matt Cranford's blog, accessed 07/08/18, at
